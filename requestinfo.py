...
class RequestInfo(object):
    ...
    def get_root_url(self):
        ...
        if config.root_url is not None:
            ...
            if 'REQUEST_URI' in self.environ:
                full_path = urllib.parse.unquote(self.environ['REQUEST_URI'])
                ...
                url += urllib.parse.quote(full_path)
    ...
    def check_if_header(self):
        ...
        if 'HTTP_IF' not in self.environ:
            return True
...
#        path = self.environ.get('PATH_INFO', '')
        # works with OpenBSD httpd to get filename
        path = self.environ.get('REQUEST_URI', '').split('/')[-1]
        path = path.strip('/')
        return path
    ...
    def parse_simple_ref(self, simple_ref):
        ...
        return urllib.parse.unquote(rel_path)
    ...
    def get_url(self, real_path):
        ...
        rel_path = urllib.parse.quote(rel_path.encode('utf-8'))
        url = urllib.parse.urljoin(self.root_url, rel_path)
...
if __name__ == '__main__':
    ...
    req = RequestInfo({
        ...
        'PATH_INFO': '/testfile%\xc3\xa4'.encode('latin1').decode('utf8'),
        'HTTP_IF': '(<' + lock.urn + '>)',
        'wsgi.input': None
    })
